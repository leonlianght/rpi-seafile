...
from utils import (
    ...
    render_template, setup_logging
)
from upgrade import check_upgrade
from bootstrap import init_seafile_server, recreate_ln
...
def main():
    if not exists(shared_seafiledir):
        os.makedirs(shared_seafiledir)
    if not exists(generated_dir):
        os.makedirs(generated_dir)

    if not init_seafile_server():
        recreate_ln()
    ...
    try:
        ...
        call('{} stop'.format(get_script('seafile.sh')))
        call('{} stop'.format(get_script('seahub.sh')))
        sys.exit(0)
